...
function FormattedMarkdown(props: React.ComponentProps<typeof Markdown>) {
  return (
    <Markdown
      components={{
        h1({ className, ...props }) {
...
const typographyComponents: Parameters<typeof Markdown>[0]['components'] = {
  ...
  ul({ className, ...props }) {
    return (
      <ul
        className={
          "my-3 [&:last-child]:mb-0 ml-6 list-disc [&>li]:mt-2 text-wrap " + className
        }
        {...props}
      />
    );
  ...
  ol({ className, ...props }) {
    return (
      <ol
        className={
          "my-3 [&:last-child]:mb-0 ml-6 list-decimal [&>li]:mt-2 text-wrap " + className
        }
        {...props}
      />
    );
  ...
  blockquote({ className, ...props }) {
    return (
      <blockquote
        className={"mt-6 border-l-2 pl-6 italic text-wrap " + className}
        {...props}
      />
    );
  },
  code({ className, ...props }) {
    return (
      <code
        className={
          "bg-muted relative rounded px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold text-wrap " +
          className
        }
        {...props}
      />
...
const MdxReactMarkdown = withMdx(Markdown);

export default function MyMarkdown({ children }: { children: string }) {
  return (
    <MdxReactMarkdown components={{ ...typographyComponents, ...mdxComponents }}>
      {children}
    </MdxReactMarkdown>
  );
}
